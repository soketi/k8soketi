...
describe('http api test', () => {
    ...
    it('get api presence channel', done => {
        let user1 = {
            user_id: 1,
            user_info: {
                id: 1,
    ...
    it('get api presence users', done => {
        let user1 = {
            user_id: 1,
            user_info: {
                id: 1,
    ...
    it('presence channel users should count only once for same-user multiple connections', done => {
        let user = {
            user_id: 1,
            user_info: {
                id: 1,
    ...
    it('passing an inexistent app id will return 404', done => {
        Utils.newServer({}, (server: Server) => {
    ...
    it('a non-presence channel cannot read users', done => {
        Utils.newServer({}, (server: Server) => {
            let client = Utils.newClient();
            let channelName = Utils.randomChannelName();
            let backend = Utils.newBackend();
    ...
    it('throw error when credentials dont match', done => {
        Utils.newServer({}, (server: Server) => {
            let client = Utils.newClient();
            let channelName = Utils.randomChannelName();
    ...
    it('should check for websockets.limits.events.maxChannelsAtOnce', done => {
        Utils.newServer({ 'websockets.limits.events.maxChannelsAtOnce': 1 }, (server: Server) => {
            let backend = Utils.newBackend();
    ...
    it('should check for websockets.limits.events.maxNameLength', done => {
        Utils.newServer({ 'websockets.limits.events.maxNameLength': 7 }, (server: Server) => {
            let backend = Utils.newBackend();
    ...
    it('should check for websockets.limits.events.maxPayloadInKb', done => {
        Utils.newServer({ 'websockets.limits.events.maxPayloadInKb': 1/1024 }, (server: Server) => {
            let backend = Utils.newBackend();
    ...
    it('should check for websockets.http.maxPayloadSizeInMb', done => {
        Utils.newServer({ 'websockets.http.maxPayloadSizeInMb': 1/1024/1024 }, (server: Server) => {
            let backend = Utils.newBackend();
    ...
    it('non existent route must return 404', done => {
        Utils.newServer({}, (server: Server) => {
    ...
    it('check server can handle a numeric app id', done => {
        Utils.newServer({
            'websockets.appManagers.drivers.array.apps.0.id': 40000
        }, (server: Server) => {
    ...
    Utils.shouldRun({ appManager: 'array' })('signin after connection with termination call', done => {
    ...
    Utils.shouldRun({ appManager: 'array' })('broadcast to user', done => {
        Utils.newServer({ 'websockets.appManagers.drivers.array.apps.0.enableUserAuthentication': true, 'userAuthenticationTimeout': 5_000 }, (server: Server) => {
            let client = Utils.newClientForPrivateChannel({}, 6001, 'app-key', { id: 1 });
            let backend = Utils.newBackend();
    ...
    it('get api presence channel with filter_by_prefix', done => {
        let user1 = {
            user_id: 1,
            user_info: {
                id: 1,
