...
describe('http api test for distributed environments', () => {
    beforeEach(() => {
        jest.resetModules();
    ...
    it('get api channels with distributed environments', done => {
    ...
    it('get api channel with distributed environments', done => {
        Utils.newServer({ port: 6001 }, (server1: Server) => {
            Utils.newClonedServer(server1, { port: 6002 }, (server2: Server) => {
                let client1 = Utils.newClient();
                let backend = Utils.newBackend();
    ...
    it('get api presence channel with distributed environments', done => {
        let user1 = {
            user_id: 1,
            user_info: {
                id: 1,
    ...
    it('get api presence users with distributed environments', done => {
        let user1 = {
            user_id: 1,
            user_info: {
                id: 1,
    ...
    it('presence channel users should count only once for same-user multiple connections with distributed environments', done => {
        let user = {
            user_id: 1,
            user_info: {
                id: 1,
    ...
    Utils.shouldRun({ appManager: 'array' })('signin after connection with termination call for cluster', done => {
        Utils.newServer({ 'websockets.appManagers.drivers.array.apps.0.enableUserAuthentication': true, 'userAuthenticationTimeout': 5_000 }, (server1: Server) => {
            Utils.newClonedServer(server1, { port: 6002, 'websockets.appManagers.drivers.array.apps.0.enableUserAuthentication': true, 'userAuthenticationTimeout': 5_000 }, (server2: Server) => {
    ...
    Utils.shouldRun({ appManager: 'array' })('broadcast to user for cluster', done => {
        Utils.newServer({ 'websockets.appManagers.drivers.array.apps.0.enableUserAuthentication': true, 'userAuthenticationTimeout': 5_000 }, (server1: Server) => {
            Utils.newClonedServer(server1, { 'port': 6002, 'websockets.appManagers.drivers.array.apps.0.enableUserAuthentication': true, 'userAuthenticationTimeout': 5_000 }, (server2: Server) => {
                let client1 = Utils.newClientForPrivateChannel({}, 6001, 'app-key', { id: 1 });
                let client2;
