...
describe('ws test', () => {
    ...
    Utils.shouldRun({ appManager: 'array' })('allows client events', done => {
        Utils.newServer({ 'websockets.appManagers.drivers.array.apps.0.enableClientMessages': true }, (server: Server) => {
    ...
    Utils.shouldRun({ appManager: 'array' })('client events for presence channels', done => {
        Utils.newServer({ 'websockets.appManagers.drivers.array.apps.0.enableClientMessages': true }, (server: Server) => {
            let user1 = {
                user_id: 1,
                user_info: {
    ...
    Utils.shouldRun({ appManager: 'array' })('client events dont get emitted when client messaging is disabled', done => {
        Utils.newServer({ 'websockets.appManagers.drivers.array.apps.0.enableClientMessages': false }, (server: Server) => {
            let client1 = Utils.newClientForPrivateChannel();
            let channelName = `private-${Utils.randomChannelName()}`;
    ...
    Utils.shouldRun({ appManager: 'array' })('client events dont get emitted when event name is big', done => {
        Utils.newServer({ 'websockets.appManagers.drivers.array.apps.0.enableClientMessages': true, 'eventLimits.maxNameLength': 25 }, (server: Server) => {
            let client1 = Utils.newClientForPrivateChannel();
            let channelName = `private-${Utils.randomChannelName()}`;
    ...
    Utils.shouldRun({ appManager: 'array' })('client events dont get emitted when event payload is big', done => {
        Utils.newServer({ 'websockets.appManagers.drivers.array.apps.0.enableClientMessages': true, 'eventLimits.maxPayloadInKb': 1/1024/1024 }, (server: Server) => {
            let client1 = Utils.newClientForPrivateChannel();
            let channelName = `private-${Utils.randomChannelName()}`;
    ...
    it('cannot connect using invalid app key', done => {
    ...
    Utils.shouldRun({ appManager: 'array' })('throw over quota error if reached connection limit', done => {
        Utils.newServer({ 'websockets.appManagers.drivers.array.apps.0.maxConnections': 1 }, (server: Server) => {
    ...
    Utils.shouldRun({ appManager: 'array' })('throw invalid app error if app is deactivated', done => {
        Utils.newServer({ 'websockets.appManagers.drivers.array.apps.0.enabled': false }, (server: Server) => {
    ...
    it('should check for websockets.limits.channels..maxNameLength', done => {
        Utils.newServer({ 'websockets.limits.channels.maxNameLength': 25 }, (server: Server) => {
            let client = Utils.newClient();
    ...
    it('should check for websockets.limits.presence.maxMemberSizeInKb', done => {
        Utils.newServer({ 'websockets.limits.presence.maxMemberSizeInKb': 1/1024/1024 }, (server: Server) => {
            let user = {
                user_id: 1,
                user_info: {
                    id: 1,
    ...
    it('should check for websockets.limits.presence.maxMembersPerChannel', done => {
        Utils.newServer({ 'websockets.limits.presence.maxMembersPerChannel': 1 }, (server: Server) => {
            let user1 = {
                user_id: 1,
                user_info: {
                    id: 1,
    ...
    it('adapter getChannelSockets works', done => {
        Utils.newServer({}, (server: Server) => {
            let client1 = Utils.newClient();
            let channelName = Utils.randomChannelName();

            client1.connection.bind('connected', () => {
                server.websocketsNode.namespace('app-id').getChannelSockets(channelName).then(sockets => {
    ...
    Utils.shouldRun({ appManager: 'array' })('signin after connection', done => {
        Utils.newServer({ 'websockets.appManagers.drivers.array.apps.0.enableUserAuthentication': true, 'userAuthenticationTimeout': 5_000 }, (server: Server) => {
    ...
    Utils.shouldRun({ appManager: 'array' })('not calling signin after connection throws right error code', done => {
        Utils.newServer({ 'websockets.appManagers.drivers.array.apps.0.enableUserAuthentication': true, 'userAuthenticationTimeout': 5_000 }, (server: Server) => {
            let client = Utils.newClientForPrivateChannel({}, 6001, 'app-key', { id: 1 });
    ...
    Utils.shouldRun({ appManager: 'array' })('not having user id throws an error', done => {
        Utils.newServer({ 'websockets.appManagers.drivers.array.apps.0.enableUserAuthentication': true, 'userAuthenticationTimeout': 5_000 }, (server: Server) => {
    ...
    Utils.shouldRun({ appManager: 'array' })('sending wrong user data token throws error', done => {
        Utils.newServer({ 'websockets.appManagers.drivers.array.apps.0.enableUserAuthentication': true, 'userAuthenticationTimeout': 5_000 }, (server: Server) => {
