...
describe('private-encrypted channel test', () => {
    ...
    it('connects to private-encrypted channel', done => {
    ...
    it('cannot connect to private-encrypted channel with wrong authentication', done => {
        Utils.newServer({}, (server: Server) => {
            let client = Utils.newClientForEncryptedPrivateChannel({
    ...
    it('connects and disconnects to private-encrypted channel and does not leak memory', done => {
        Utils.newServer({}, (server: Server) => {
            let client = Utils.newClientForEncryptedPrivateChannel();
            let backend = Utils.newBackend();
            let channelName = `private-encrypted-${Utils.randomChannelName()}`;
    ...
    it('sudden close connection in private-encrypted channel and does not leak memory', done => {
        Utils.newServer({}, (server: Server) => {
            let client = Utils.newClientForEncryptedPrivateChannel();
            let backend = Utils.newBackend();
            let channelName = `private-encrypted-${Utils.randomChannelName()}`;
    ...
    it('cached private-encrypted channels work', done => {
        Utils.newServer({}, (server: Server) => {
