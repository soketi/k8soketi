...
describe('presence channel test', () => {
    ...
    it('connects to presence channel', done => {
    ...
    it('handles joins and leaves', done => {
        Utils.newServer({}, (server: Server) => {
            let john = {
                user_id: 1,
                user_info: {
    ...
    it('cannot connect to presence channel with wrong authentication', done => {
        Utils.newServer({}, (server: Server) => {
            let user = {
                user_id: 1,
                user_info: {
    ...
    it('connects and disconnects to presence channel and does not leak memory', done => {
        Utils.newServer({}, (server: Server) => {
            let user = {
                user_id: 1,
                user_info: {
    ...
    it('sudden close connection in presence channel and does not leak memory', done => {
        Utils.newServer({}, (server: Server) => {
            let user = {
                user_id: 1,
                user_info: {
    ...
    it('connecting to the channel twice with same user does not trigger member_added twice', done => {
        Utils.newServer({}, (server: Server) => {
            let john = {
                user_id: 1,
                user_info: {
                ...
                johnChannel.bind('pusher:subscription_succeeded', (data) => {
                    ...
                    aliceClient.connection.bind('connected', () => {
                        ...
                        aliceChannel.bind('pusher:member_removed', data => {
                            server.websocketsNode.namespace('app-id').getChannelMembers(channelName).then(members => {
                                if (members.size === 1) {
                                    done();
                                    aliceClient.disconnect();
                                } else {
    ...
    it('cached presence channels work', done => {
        Utils.newServer({}, (server: Server) => {
            let john = {
                user_id: 1,
                user_info: {
    ...
    Utils.shouldRun({ appManager: 'array' })('user authentication works if conn immediately joins a presence channel', (done) => {
