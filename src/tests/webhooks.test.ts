...
describe('webhooks test', () => {
    ...
    it('webhooks from client events', done => {
        let webhooks = [{
        ...
        Utils.newServer({
            'websockets.appManagers.drivers.array.apps.0.enableClientMessages': true,
            'websockets.appManagers.drivers.array.apps.0.webhooks': webhooks,
        }, (server: Server) => {
            Utils.newWebhookServer((req, res) => {
                let app = new App(server.options.websockets.appManagers.drivers.array.apps[0], server.options);
                let rightSignature = app.hmac(JSON.stringify(req.body));
    ...
    it('webhooks from channel_occupied and channel_vacated', done => {
        let webhooks = [{
    ...
    it('webhooks from member_added and member_removed', done => {
        let webhooks = [{
    ...
    it('webhooks from member_added and member_removed alongside filtering', done => {
        let channelName = `presence-${Utils.randomChannelName()}`;
    ...
    it('webhooks filtered by channel name', done => {
    ...
    it('webhooks can have custom headers', done => {
        const webhooks = [{
            event_types: ['channel_occupied'],
            url: 'http://127.0.0.1:3001/webhook',
            headers: {
    ...
    Utils.shouldRun({ queueManager: 'sqs' })('sqs batching', done => {
        let webhooks = [{
            event_types: ['client_event'],
            url: 'http://127.0.0.1:3001/webhook',
        }];
        ...
        Utils.newServer({
            'websockets.appManagers.drivers.array.apps.0.enableClientMessages': true,
            'websockets.appManagers.drivers.array.apps.0.webhooks': webhooks,
            'websockets.queueManagers.sqs.processBatch': true,
            'websockets.queueManagers.sqs.batchSize': 2,
            'websockets.queueManagers.sqs.pollingWaitTimeMs': 1000,
        }, (server: Server) => {
            Utils.newWebhookServer((req, res) => {
                let app = new App(server.options.websockets.appManagers.drivers.array.apps[0], server.options);
                let rightSignature = app.hmac(JSON.stringify(req.body));
    ...
    it('webhooks from cache_miss', done => {
        let webhooks = [{
    ...
    it('webhooks batching works', done => {
        let webhooks = [{
            event_types: ['client_event'],
            url: 'http://127.0.0.1:3001/webhook',
        }];
        ...
        Utils.newServer({
            'websockets.appManagers.drivers.array.apps.0.enableClientMessages': true,
            'websockets.appManagers.drivers.array.apps.0.webhooks': webhooks,
            'websockets.webhooks.batching.enabled': true,
            'websockets.webhooks.batching.duration': 3000,
        }, (server: Server) => {
            Utils.newWebhookServer((req, res) => {
                let app = new App(server.options.websockets.appManagers.drivers.array.apps[0], server.options);
                let rightSignature = app.hmac(JSON.stringify(req.body));
